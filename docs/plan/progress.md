...
- [x] 15.12 Firebase App Distribution ("App Tester") + esteira de release no merge para `main` — `.github/workflows/release.yml` (gates → bump semver via Conventional Commits → build APK → distribui ao grupo `qa` → commita nova versão no `pubspec.yaml` com `[skip ci]`, sem loop) + script [`tools/release/bump_version.sh`](../../tools/release/bump_version.sh) + doc [docs/release/firebase-app-distribution.md](../release/firebase-app-distribution.md). **Falta (humano):** habilitar App Distribution no console, criar grupo `qa` + e-mails, configurar secrets (`FIREBASE_SERVICE_ACCOUNT`, keystore, `RELEASE_TOKEN` se `main` protegida), validar 1 build manual via CLI. iOS adiado. ([issue #24](https://github.com/4rthurmonteiro/onebit-dice/issues/24))
